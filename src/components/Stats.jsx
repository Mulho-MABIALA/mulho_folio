...
const StatCard = ({ stat, index, visible }) => {
  ...
  return (
    <div
      className="group relative bg-zinc-900/50 backdrop-blur-sm rounded-xl p-6 transition-all duration-500 hover:-translate-y-2"
      style={{
      ...
      onMouseEnter={e => {
        ...
        e.currentTarget.style.boxShadow = '0 25px 50px rgba(0,0,0,0.4), 0 0 30px rgba(var(--accent-rgb), 0.15)'
      }}
      onMouseLeave={e => {
    ...
    >
      ...
      <div
        ...
        style={{ background: 'radial-gradient(circle at 50% 0%, rgba(var(--accent-rgb), 0.08), transparent 70%)' }}
      />

      {/* Icône */}
      <div className="relative mb-4">
        <div
          className="inline-flex items-center justify-center w-14 h-14 rounded-lg group-hover:scale-110 transition-transform duration-300"
          style={{ background: 'rgba(var(--accent-rgb), 0.15)' }}
        >
          <div style={{ color: 'var(--accent-400)' }}>{stat.icon}</div>
        </div>
      </div>
      ...
      <div className="relative mb-2">
        <h3
          className="text-3xl sm:text-4xl font-bold group-hover:scale-105 transition-transform duration-300 inline-block"
      ...
      <div
        ...
        style={{ background: 'linear-gradient(to right, var(--accent-400), var(--accent-y500))' }}
      />
    </div>
  )
}
...
const Stats = () => {
  ...
  const stats = [
    { icon: <FaProjectDiagram className="w-7 h-7" />, number: '12+', label: 'Projets d\'excellence' },
    { icon: <FaBriefcase className="w-7 h-7" />, number: '3+', label: 'Années d\'expertise' },
    { icon: <FaThumbsUp className="w-7 h-7" />, number: '95%', label: 'Satisfaction client' },
    { icon: <FaUsers className="w-7 h-7" />, number: '4', label: 'Collaborateurs' },
  ]
